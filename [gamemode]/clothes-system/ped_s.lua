...
local function getTemporaryPath(url)
	return 'sarp_cache_temp/' .. encryptFileNameServer(url) .. '.temp'
end
...
function wizard2Result(url, desc, skin_, for_faction)
	...
	-- 			outputDebugString( tostring(size) )
	-- 			local maxSize = 300000 --300kb
	-- 			-- if exports.global:hasPlayerAccess(player, "premium_gold") then
	-- 			-- 	maxSize = 400000 --400kb
	-- 			-- elseif exports.global:hasPlayerAccess(player, "premium_silver") then
	-- 			-- 	maxSize = 300000 --300kb
	-- 			-- elseif exports.global:hasPlayerAccess(player, "premium_bronze") then
	-- 			-- 	maxSize = 200000 --200kb
	-- 			-- end
	-- 			if size > maxSize then
	-- 				triggerClientEvent(player, 'clothes:wizard2Result', resourceRoot, 'Image exceeds the maximum size of '..tostring(maxSize/1000)..'kb')
	-- 			elseif height > 500 or width > 1200 then
	-- 				triggerClientEvent(player, 'clothes:wizard2Result', resourceRoot, 'Image exceeds the maximum dimensions of 500x1200')
	-- 			else
	-- 				-- Initial Checks done
	-- 				local options = {
	-- 					queueName = "clothes",
	...
	-- 								if fid then
	-- 									add.creator_charname = exports["faction-system"]:getFactionName(fid)
	...
	fetchRemote(url, options, function(str, errno)
		if not errno.success then
			...
			local file_path = getTemporaryPath(url)
			...
			local maxSize = 300 * 1024
			if size > maxSize then
				triggerClientEvent(player, 'clothes:wizard2Result', resourceRoot, 'Image exceeds the maximum size of 300 kb')
				fileDelete(file_path)
			else
				...
				if exports.global:takeMoney(player, 25) then
					...
					local add = {
									url=url_,
									description=desc_,
									skin=skin, price=defaultSkinCost,
									creator_charname=exports.global:getPlayerName(player),
									creator_char=getElementData(player, 'dbid'),
									for_sale_until=0,
									date=exports.datetime:now(),
						...
						if fileExists(path) then
							...
							if file then
								...
								-- if not instant then
									...
									-- extra_minutes = math.random(10, 20) -- math.ceil(size/one_minute_size)
								-- else
								...
								done, why = true, "Your new clothes design is now ready!"
								triggerClientEvent(client, 'clothes:callback_Manu', resourceRoot, done, why)
								fileClose(file)
								-- send notification when it's arrived
								local content = "Clothes manufactoring request #"..clothing.id.." Update"
								sendBatchNotis(getElementData(client, 'account:id'), content)
	...
	if cl then
		if cl.creator_char == pid then
			...
			if cl.distribution == 2 and not (cl.mdate and cl.mdate > 0 and cl.mdate > exports.datetime:now()) then
				...
				why = "Your design will now be distributed globally."
				id = cl.id
				dist = 3
			end
		end
...
addEventHandler( 'clothes:deleteMyClothes', resourceRoot, function( index )
	...
	if clothes then
		...
		local qh = dbQuery( exports.mysql:getConn('mta'), "SELECT `index` FROM items WHERE itemID=16 and itemValue=? and destroyed=0 LIMIT 1", search_for )
		local result, num_affected_rows, last_insert_id = dbPoll ( qh, 10000 )
		if num_affected_rows > 0 then
			triggerClientEvent( client, 'clothes:deleteMyClothes', resourceRoot, index, false, "There are one or more clothes items in some inventory using this design. Find and destroy them first." )
		else
			local qh = dbQuery( exports.mysql:getConn('mta'), "SELECT `id` FROM worlditems WHERE itemid=16 and itemvalue=? and deleted=0  LIMIT 1", search_for )
			local result, num_affected_rows, last_insert_id = dbPoll ( qh, 10000 )
			if num_affected_rows > 0 then
				triggerClientEvent( client, 'clothes:deleteMyClothes', resourceRoot, index, false, "There are one or more clothes items existing (dropped) using this design. Find and destroy them first." )
			else
				...
				local result, num_affected_rows, last_insert_id = dbPoll ( qh, 10000 )
				if num_affected_rows > 0 then
					triggerClientEvent( client, 'clothes:deleteMyClothes', resourceRoot, index, false, "There are one or more clothes items for sale in a shop using this design. Find and destroy them first." )
				else
...
function obtainSkinItem(skinid, cost, title)

	if exports.global:hasMoney(client, cost) then
		if exports.global:giveItem(client, 16, skinid) then
			exports.global:takeMoney(client, cost)
			outputChatBox("You've received a clothes item for Skin ID #"..skinid..".", client, 0, 255, 0)
			outputChatBox("Name: "..title, client, 255, 194, 0)
		else
			outputChatBox("You don't have space in your inventory to receive a clothes item.", client, 255, 0, 0)
		end
	else
		outputChatBox("You need $"..cost.." to obtain this clothes item.", client, 255, 0, 0)
	end
end
addEvent("obtainSkinItem", true)
addEventHandler("obtainSkinItem", root, obtainSkinItem)
