...
function resetPed()
	...
	if previewPed and isElement(previewPed) then
		setElementData(previewPed, "skinID", nil)
...
function closeWindow()
	if window then
		...
		window = nil
		setSoundVolume(playSound(":resources/inv_close.aac"), 0.3)
		triggerEvent('npc:togShopWindow', resourceRoot, true)
		removeEventHandler('onClientCharacterLogout', localPlayer, closeWindow)
	end
	closeEditingWindow()
	resetPed()
end
...
function updateGrid(dopont_npc)
	...
	if not dopont_npc then
		...
		if moddedSkins then
			...
			for k, v in pairs(moddedSkins) do
				if tonumber(v.modelid) == tonumber(selected_skin) then
					info = moddedSkins[k]
					break
				end
			end
		...
		if not info then
			local v = {id=0, description="Original", creator_charname=getGtaDesigners(), creator_char=0, price=defaultSkinCost, skin=selected_skin, date="Unknown"}
			...
			guiGridListSetItemText(grid, row, 1, 0, false, true)
			guiGridListSetItemText(grid, row, 2, info.title, false, false)
			guiGridListSetItemText(grid, row, 3, tostring(selected_skin), false, true)
			guiGridListSetItemText(grid, row, 4, '$' .. exports.global:formatMoney(defaultSkinCost), false, false)
			guiGridListSetItemText(grid, row, 5, info.uploadDate , false, false)
		end
	end
...
function listClothes(item, list)
	...
	if window and isElement(window) then
		...
		local margin = 30
		window = guiCreateWindow(screen_width - width - 45, screen_height - height - 20, width, height, "Collection #"..(item.itemValue or ""), false)
				...
				if dopont_npc then
					if cl then
						if row == -1 then
							...
							if cl then
								if exports["sarp-new-mods"]:isCustomMod(cl.skin, "ped") then
									setElementData(default_ped.ped, "skinID", cl.skin)
								else
					...
					if row == -1 then
						...
						if row == 0 then -- default skin
							if exports["sarp-new-mods"]:isCustomMod(selected_skin, "ped") then
								setElementData(default_ped.ped, "skinID", selected_skin)
							else
								setElementData(default_ped.ped, "skinID", nil)
								setElementModel(default_ped.ped, selected_skin)
							end
							setElementData(default_ped.ped, 'clothing:id', nil , false)
							guiSetEnabled(buy, exports.global:hasMoney(localPlayer, exports.global:roundNumber(defaultSkinCost)))
							setPedAnimation ( default_ped.ped )
						elseif cl then
							if exports["sarp-new-mods"]:isCustomMod(cl.skin, "ped") then
								setElementData(default_ped.ped, "skinID", cl.skin)
							...
							setElementData(default_ped.ped, 'clothing:id', cl.id , false)
							guiSetEnabled(buy, exports.global:hasMoney(localPlayer, exports.global:roundNumber(cl.price)))
							selected_skin = cl.skin
							setPedAnimation ( default_ped.ped )
						else
							guiSetEnabled(buy, false)
			...
			function(button)
				if button == 'left' then
					...
					if row ~= -1 then
						...
						local _ped,_sel
						if row == 0 then -- default skin
							_ped,_sel = default_ped.ped, -selected_skin
						elseif item then
							_ped,_sel = default_ped.ped, item.id
						end
						closeWindow()
						
						if _ped and _sel then
							triggerServerEvent('clothing:buy', _ped,_sel)
						end
					end
				end
		end, false)
