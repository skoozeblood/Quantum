...
local blipPath							= "blips/"
local mapFile                           = imgPath.."/hd_1.png"
...
function drawBlips()
	for k,v in ipairs(getElementsByType("blip")) do
		if not getElementData(v,"DoNotDrawOnMaximap") then
			...
			dxDrawImage(x-halfsize,y-halfsize,size,size,imgPath..blipPath..icon..".png",0,0,0,tocolor(r,g,b,a),false)
		end
	end
end
...
function drawLocalPlayerArrow()
	...
	dxDrawImage(mapX-8,mapY-8,blipSize,blipSize,imgPath..blipPath.."2.png",(-r)%360,0,0,normalColor,false)
end
	...
	if mapShowing then
		if pressed then
			...
			if key == "mouse_wheel_up" then
				makeMovement("zoom_in")
			elseif key == "mouse_wheel_down" then
				makeMovement("zoom_out")
			end
		end
	end
end)

function makeMovement(str)
	...
	if isCursorShowing() then
		...
		if cursorX and cursorY then
			...
			if str == "zoom_in" then
				...
				if zoom<maxZoomLimit then
					zoom=zoom+zoomRate
					if zoom>maxZoomLimit then zoom=maxZoomLimit end
				end
			elseif str == "zoom_out" then
				if zoom>minZoomLimit then
					zoom=zoom-zoomRate
...
function setBlipShowingOnMaximap(blip, tog)
	if isElement(blip) and getElementType(blip)=="blip" and type(tog)=="boolean" then
		return setElementData(blip,"DoNotDrawOnMaximap",not tog,false)
	end
	return false
end
