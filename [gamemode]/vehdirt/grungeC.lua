...
function handleDirtShader(vehicle, level)
	...
	if isValidDirtVehicle(vehicle) then
		...
		if not grungeVehicles[vehicle] or grungeVehicles[vehicle] == nil then
			...
			if grungeVehicles[vehicle].shader and isElement(grungeVehicles[vehicle].shader) then
		...
		-- no need to apply shader for clean car / Fernando
        ...
        if getElementType(source) == "vehicle" then
			if isValidDirtVehicle(source) then
				if not grungeVehicles[source] or grungeVehicles[source] == nil then
					if not getElementData(source, "clientShopVeh") then
...
addEventHandler("onClientElementStreamOut", getRootElement(),
    function()
        if getElementType(source) == "vehicle" then
			if isValidDirtVehicle(source) then
				if grungeVehicles[source] and isElement(grungeVehicles[source].shader) then
