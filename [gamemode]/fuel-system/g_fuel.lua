...
local fd = getElementData(source, "fueldata")
if fd and table.size(fd) > 0 then
	outputChatBox(fd.cap, thePlayer)
	outputChatBox(fd.con, thePlayer)
end

]]
function getMaxFuel(id)
	if isElement(id) then
		id = exports.global:getVehicleModelNew(id)
	end
