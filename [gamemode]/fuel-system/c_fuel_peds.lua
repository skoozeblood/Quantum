...
function onQuestionShow(questionArray)
	...
	if not (wGui) then
		...
		for answerID, answerStr in ipairs(questionArray) do
			if (answerStr) then
				local aheight = 0.2
				if string.find(answerStr, "Fill her") then
					aheight = 0.1
				end
				local option = guiCreateButton( 0.05, verticalPos, 0.9, aheight, answerStr, true, wGui )
...
function requestATMInterfacePIN(theATM, atmLocationName)
	selectedElement = source
	if getElementData(getLocalPlayer(), "exclusiveGUI") or isPedDead(localPlayer) or getElementData(localPlayer, "isdown") then
		return false
	end
	if not (wPIN) then
		...
		Label_Error = guiCreateLabel(20,90,220,30,"Please enter 4 digits of PIN Code",false,tabPersonal)
...
function respondToATMInterfacePIN(Label_Error_Msg, r,g,b, action, otherInfo)
	...
	elseif action == "locked" then
		exports.hud:sendBottomNotification(localPlayer, "Point of Sale Machine", "Debit Card Number '"..otherInfo.."' is locked and not usable, please contact the Bank.")
		guiSetEnabled(bEnter, true)
