function submitExteriorMapRequest ( name, url, who, what, why, map, player, converter )

	-- for some super weird reasons, mta built in mysql function refuses to work for half of the records if you loop it instantly, no idea why.
	...
	if res and map_id then
		for _, obj in ipairs( map ) do
			dbExec( exports.mysql:getConn('mta'), "INSERT INTO maps_objects SET map_id=?, id=?, interior=?, dimension=?, collisions=?, breakable=?, radius=?, model=?, lodModel=?, posX=?, posY=?, posZ=?, rotX=?, rotY=?, rotZ=?, doublesided=?, scale=?, alpha=?, textures=?, distance=?", map_id, obj.id, obj.interior, obj.dimension, obj.collisions, obj.breakable, obj.radius, obj.model, obj.lodModel, obj.posX, obj.posY, obj.posZ, obj.rotX, obj.rotY, obj.rotZ, obj.doublesided, obj.scale, obj.alpha, obj.textures, obj.distance)
		end
		return true
	else
		dbFree( qh )
...
-- Fernando
function submitInteriorMapRequest ( name, url, who, what, why, map, player )

	local qh, res, nums, map_id

	qh = dbQuery( exports.mysql:getConn('mta'), "INSERT INTO maps SET type=?, name=?, preview=?, purposes=?, used_by=?, reasons=?, uploader=?, note='' ", "interior", name, url, what, who, why, getElementData( player, 'account:id' ) )
	res, nums, map_id = dbPoll( qh, 10000 )
	...
	if res and map_id then
		...
		dbFree( qh )
		return false, 'Internal Error. Code 18.'
	end
end
	...
	if res then
		if nums > 0 then
			if res[1].id and res[1].id > 0 then
				...
				exports.announcement:makePlayerNotification( res[1].id, subject, content )
			end
		end
	else
		dbFree( qh )
...
---addCommandHandler( 'exportinteriormap', function ( player, cmd, dim )
	--[[if ( exports.integration:isPlayerSeniorAdmin( player ) or exports.integration:isPlayerScripter( player ) ) and getElementData( player, 'loggedin' ) == 1 then
...
addCommandHandler( 'exportmap', function ( player, cmd, e_type, mapid )
	if ( exports.integration:isPlayerSeniorAdmin( player )
	or exports.integration:isPlayerScripter( player )
	or exports.integration:isPlayerMTLeader( player ) )
	and getElementData( player, 'loggedin' ) == 1 then

		if e_type and tonumber(e_type) and tonumber(e_type) >= 1 and tonumber(e_type) <= 2 and mapid and tonumber(mapid) and tonumber(mapid) > 0 then
			dbQuery( function( qh, player, mapid )
				local res , nums, inserted = dbPoll ( qh, 0 )
				if res then
					if nums > 0 then
						local export_type = "map"
						if tonumber(e_type) == 2 then
							export_type = "lua"
						end

						triggerClientEvent( player, 'map:exportmap', resourceRoot, res, export_type )
						exports.logs:dbLog( player, 4, player, cmd.." "..export_type.." #"..mapid )
					else
			...
			outputChatBox( "SYNTAX: /"..cmd.." [(1) .map | (2) .lua] [Map ID from /maps]", player )
		end
	end
end, false, false )


--[[ Absolutely no reason to use - Portside
